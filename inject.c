...
struct root *get_all_module_r(pid_t pid) {
    ...
    fp = fopen(filename, "r");
    if (fp != NULL) {
        link = malloc(sizeof(struct root));
...
//        printf("mylibc addr:%p  -- mproject:%p", local_libc_addr, (void *) mprotect);
        void *remote_mproject = (uintptr_t) (void *) mprotect - (uintptr_t) local_libc_addr
                                + libc_node.begin;
        ...
        for (int i = 0; i < link->length; i++) {
            ...
            if (switch_statue) {
                for (; tmp_base < tmp_end; tmp_base++) {
                    ...
                    if (strcmp(str, ro) == 0) {
                        ptrace_setData(target_pid, tmp_base, sd, 4);
                    }
                }
            } else {
                for (; tmp_base < tmp_end; tmp_base++) {
                    str = ptrace_getData(target_pid, tmp_base, 4);
                    if (strcmp(str, sd) == 0) {
                        ptrace_setData(target_pid, tmp_base, ro, 4);
                    }
                }
            }
